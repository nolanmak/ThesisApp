...
class AudioWebSocketService {
  ...
  public connect(): void {
    ...
    if (this.isConnecting) {
      console.log('Audio WebSocket already connecting, skipping duplicate connect attempt');
      return;
    }
    ...
    try {
      ...
      if (this.socket) {
        try {
          ...
          if (this.socket.readyState === WebSocket.OPEN || 
            ...
            console.log(`Closing existing Audio WebSocket in state: ${this.socket.readyState}`);
            this.socket.close();
          }
        } catch (e) {
      ...
      console.log('Creating new Audio WebSocket connection to', AUDIO_WS_ENDPOINT);
      ...
      this.socket.onmessage = (event) => {
        try {
          ...
          if (data.type === 'new_audio') {
            ...
            console.warn('Received Audio WebSocket message with unknown format:', data);
          }
        } catch (error) {
      ...
      this.socket.onerror = (error) => {
        ...
        if (this.reconnectAttempts >= this.maxReconnectAttempts - 1) {
          toast.error('Connection to the audio service is failing. The service may be temporarily unavailable.');
        }
  ...
  public disconnect(): void {
    ...
    if (this.socket) {
      try {
        ...
        if (this.socket.readyState === WebSocket.OPEN || 
            this.socket.readyState === WebSocket.CONNECTING) {
          console.log(`Closing Audio WebSocket in state: ${this.socket.readyState}`);
          this.socket.close();
        } else {
          console.log(`Not closing Audio WebSocket - already in state: ${this.socket.readyState}`);
        }
      } catch (e) {
  ...
  public subscribeToAudioNotifications(handler: AudioMessageHandler): () => void {
  ...
  private attemptReconnect(): void {
    ...
    this.reconnectTimeout = window.setTimeout(() => {
      console.log(`Attempting to reconnect to Audio WebSocket (attempt ${this.reconnectAttempts})`);
      this.connect();
      this.reconnectTimeout = null;
    }, delay);
  }
