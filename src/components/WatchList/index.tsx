...
import { List, Upload, Trash2, ToggleLeft, ToggleRight } from 'lucide-react';
...
const WatchList: React.FC = () => {
  ...
  // Update local state when watchlist changes
  useEffect(() => {
    if (watchlist.length > 0) {
      setTickers(watchlist.join('\n'));
    } else {
      setTickers('');
    }
  }, [watchlist]);
  ...
  const updateWatchlist = async (watchlistArray: string[]) => {
    ...
    const newSet = new Set(watchlistArray);
    
    // Remove tickers not in new list
      ...
      <div className="bg-white dark:bg-neutral-800 rounded-lg shadow-sm border border-neutral-200 dark:border-neutral-700 p-6">
            ...
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-2">
                <span className="text-sm text-neutral-600 dark:text-neutral-400">
                  {watchListOn ? 'Filter On' : 'Filter Off'}
                </span>
                <button
        ...
        <div className="space-y-4">
          ...
          <div className="flex justify-between">
            ...
            >
              ...
              {loading ? 'Updating...' : 'Update Watch List'}
            </button>
          </div>
        </div>
        ...
        {watchlist.length > 0 && (
          <div className="mt-6 p-4 bg-blue-50 dark:bg-blue-900/30 rounded-md border border-blue-200 dark:border-blue-800">
            <h3 className="text-sm font-medium text-blue-700 dark:text-blue-400 mb-2">Current Watch List ({watchlist.length} tickers):</h3>
